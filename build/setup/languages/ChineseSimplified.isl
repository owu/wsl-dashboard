...
CreateSchedulerTask=创建任务计划程序（用于发行版自启、USB自动连接、端口转发激活）
SchedulerTaskNote1=└ 安装时可选，暂不勾选也不影响后续使用；
SchedulerTaskNote2=└ 可在软件打开后随时触发自动创建；
SchedulerTaskNote3=└ 因无EV代码签名证书，该操作可能被杀毒软件误报，请允许本次操作。
CreateDesktopIcon=创建桌面快捷方式
LaunchProgram=运行 %1
CleanupData=删除用户配置文件和日志：~\.wsldashboard
StartMenuIcon=创建开始菜单程序组（包含启动和卸载快捷方式）
VisitWebsite=访问官方网站
UninstallerName=%1 卸载程序
